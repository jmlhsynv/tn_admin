...
export const deleteMark = createAsyncThunk("marks/deleteMark", async (id) => {
  const headers = {
    "Content-Type": "application/json",
    Authorization: "Bearer " + localStorage.getItem("token"),
  };
        ...
        return {
          ...
          marks: [
            ...state.marks,
            {
              ...
              CATEGORY_NAME: payload.success.CATEGORY_NAME,
    ...
    [deleteMark.fulfilled]: (state, { payload }) => {
      if (payload.success === true) {
        return {
          ...state,
          pending: false,
          marks: [...state.marks.filter((e) => e.ID !== payload.id)],
          error: "success",
        };
